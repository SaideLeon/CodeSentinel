    ...
    <div className="min-h-screen bg-[#070b12] text-slate-100 antialiased">
      ...
      <div className="relative flex min-h-screen">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8">
          ...
          {activeView === 'workbench' && <section className="grid gap-6 xl:grid-cols-[360px,1fr]">
          ...
          {activeView === 'repository' && <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-6"><h2 className="text-xl font-black">Mapa do repositório</h2><div className="mt-5 grid gap-3">{(report?.blueprint.repositoryMap || files.map(f => ({ path: f.path, language: f.language, lines: f.content.split('\n').length, findings: 0 }))).map(file => <div key={file.path} className="grid gap-2 rounded-2xl border border-white/10 bg-slate-950 p-4 text-sm md:grid-cols-[1fr,120px,100px,120px]"><span className="font-mono text-slate-200">{file.path}</span><span className="text-slate-400">{file.language}</span><span className="text-slate-400">{file.lines} linhas</span><span className="font-bold text-cyan-300">{file.findings} findings</span></div>)}</div></section>}
